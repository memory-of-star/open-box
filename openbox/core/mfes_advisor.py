...
from openbox.core.base import build_acq_func, build_optimizer, build_surrogate, build_fidelity_surrogates


class MFES_Advisor(object, metaclass=abc.ABCMeta):
    """
    ...
    def __init__(
        ...
        self.fidelity_histories = []
        for i in range(self.fidelity_num):
            self.fidelity_histories.append(History(
            task_id=task_id, num_objectives=num_objectives, num_constraints=num_constraints, config_space=config_space,
            ref_point=ref_point, meta_info=None,  # todo: add meta info
        ))
        ...
        self.surrogate_fidelity_models = None
        self.fidelity_acquisition_functions = None
        self.fidelity_optimizers = None
        self.fidelity_constraint_models = None
    ...
    def check_setup(self):
        ...
        if self.num_objectives == 1:
            if self.num_constraints == 0:
                assert self.acq_type in ['ei', 'eips', 'logei', 'pi', 'lcb', 'lpei', 'mfei']
            else:  # with constraints
    ...
    def setup_bo_basics(self):
        ...
        An optimizer object.
        """
        # instead of self.surrogate
        if self.num_objectives == 1 or self.acq_type == 'parego':
            self.surrogate_fidelity_models = build_fidelity_surrogates(func_str=self.surrogate_type,
                                                   config_space=self.config_space,
                                                   rng=self.rng,
                                                   transfer_learning_history=self.transfer_learning_history,
                                                   fidelity_num=self.fidelity_num)
        else:  # multi-objectives
            self.surrogate_fidelity_models = []
            for i in range(self.fidelity_num):
                surrogate_fidelity_models_ = [build_surrogate(func_str=self.surrogate_type,
                                                        config_space=self.config_space,
                                                        rng=self.rng,
                                                        transfer_learning_history=self.transfer_learning_history)
                                        for _ in range(self.num_objectives)]
                self.surrogate_fidelity_models.append(surrogate_fidelity_models_)

        # instead of self.constraint_models
        if self.num_constraints > 0:
            self.fidelity_constraint_models = []
            for i in range(self.fidelity_num):
                constraint_models = [build_surrogate(func_str=self.constraint_surrogate_type,
                                                        config_space=self.config_space,
                                                        rng=self.rng) for _ in range(self.num_constraints)]
                self.fidelity_constraint_models.append(constraint_models)
        else:
            self.fidelity_constraint_models = []
            for i in range(self.fidelity_num):
                self.fidelity_constraint_models.append(None)
        
        # instead of self.acquisition_function
        if self.acq_type in ['mesmo', 'mesmoc', 'mesmoc2', 'usemo']:
            self.fidelity_acquisition_functions = []
            for i in range(self.fidelity_num):
                self.fidelity_acquisition_functions.append(build_acq_func(func_str=self.acq_type,
                                                        model=self.surrogate_fidelity_models[i],
        ...
        # instead of self.optimizer
        self.fidelity_optimizers = []
        for i in range(self.fidelity_num):
            self.fidelity_optimizers.append(build_optimizer(func_str=self.acq_optimizer_type,
                                            acq_func=self.fidelity_acquisition_functions[i],
                                            config_space=self.config_space,
                                            rng=self.rng))
    ...
    def get_suggestion(self, history: History = None, return_list: bool = False):
        ...
        fidelity_num_config_evaluated = []
        fidelity_num_config_successful = []
        ...
        for i in range(self.fidelity_num):
            if fidelity_num_config_successful[i] > 0:
                X.append(self.fidelity_histories[i].get_config_array(transform='scale'))
                Y.append(self.fidelity_histories[i].get_objectives(transform='infeasible'))
                cY.append(self.fidelity_histories[i].get_constraints(transform='bilog'))
            else:
                X.append(None)
                Y.append(None)
                cY.append(None)
        ...
        if self.optimization_strategy == 'bo':
            ...
            if self.num_objectives == 1:
                for i in range(self.fidelity_num):
                    if fidelity_num_config_successful[i] > 0:
                        self.surrogate_fidelity_models[i].train(X[i], Y[i][:, 0])
            elif self.acq_type == 'parego':
                for i in range(self.fidelity_num):
                    if fidelity_num_config_successful[i] > 0:
                        weights = self.rng.random_sample(self.num_objectives)
                        weights = weights / np.sum(weights)
                        scalarized_obj = get_chebyshev_scalarization(weights, Y[i])
            ...
            for k in range(self.fidelity_num):
                if fidelity_num_config_successful[k] > 0:
                    for i in range(self.num_constraints):
                        self.fidelity_constraint_models[k][i].train(X[k], cY[k][:, i])
                ...
                if self.acq_type == 'parego':
                    ...
                    for i in range(self.fidelity_num):
                        if fidelity_num_config_successful[i] > 0:
                            self.fidelity_acquisition_functions[i].update(model=self.surrogate_fidelity_models[i],
                                                            constraint_models=self.fidelity_constraint_models[i],
                                                            constraint_perfs=cY[i],  # for MESMOC
                                                            eta=fidelity_mo_incumbent_values[i],
                                                            num_data=fidelity_num_config_evaluated[i],
                                                            X=X[i], Y=Y[i])
    ...
    def get_history(self):
        import copy
        histories = copy.deepcopy(self.fidelity_histories)
        # histories = [None] + histories # 这是一个技术债，为了方便在前面补一个None
        return histories
